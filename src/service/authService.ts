...
export const login = async function(credentials:Credentials):Promise<string> {
    try{
        const response = await axios.post("http://13.49.21.183:8080/api/login", credentials);
...
export const register = async function(user: User): Promise<void> {
    try {
        await axios.post("http://13.49.21.183:8080/api/register", user);
    } catch (e) {
...
export const whoami = async function(token: string): Promise<User> {
    try {
        const response = await axios.get("http://13.49.21.183:8080/api/whoami", {
