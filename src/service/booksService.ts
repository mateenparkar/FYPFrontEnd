...
export const viewBooks = async function (): Promise<BookWithAuthor[]> {
    try {
        const booksResponse = await axios.get('http://13.49.21.183:8080/api/books');
        const books: Books[] = booksResponse.data;

        const booksWithAuthors: Promise<BookWithAuthor>[] = books.map(async (book: Books) => {
            const authorResponse = await axios.get(`http://13.49.21.183:8080/api/author/${book.author}`);
            const authorName: string = authorResponse.data.name;

            const genreResponse = await axios.get(`http://13.49.21.183:8080/api/genre/${book.genre}`);
...
export const viewBook = async function (id: number): Promise<BookWithAuthor> {
    try {
        const bookResponse = await axios.get('http://13.49.21.183:8080/api/books/' + id);
...
export const likeBook = async function(book:Book): Promise<void> {
    try{
        await axios.post("http://13.49.21.183:8080/api/addBookToUser", book);
    }catch(e){
...
export const deleteBook = async function(book:Book):Promise<void>{
    try{
        ...
        await axios.delete("http://13.49.21.183:8080/api/deleteBookFromUser", { data: book });
    }catch(e){
        console.log(e)
...
export const getLikedBooks = async function(id:number):Promise<BookWithAuthor[]>{
    try{
        const response = await axios.get('http://13.49.21.183:8080/api/getUserBooks/' + id);
...
export const updateUserBook = async function(book:userBooks):Promise<void>{
    try{
        await axios.post("http://13.49.21.183:8080/api/updateBookForUser", book);
    }catch(error){
...
export const hasUserReadBook = async function(userId: number, bookId: number): Promise<boolean> {
    try {
        const response = await axios.get(`http://13.49.21.183:8080/api/getBookDetails/${userId}/${bookId}`);
